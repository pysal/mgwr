"""
Tests for the custom GWR kernel class
"""
...
class TestKernels(unittest.TestCase):
    def setUp(self):
        ...
        self.fix_bisquare_kern = np.array(
            [[1., 0., 0., 0.,
              0.60493827], [0., 1., 0.60493827, 0.01234568, 0.],
             [0., 0.60493827, 1., 0.60493827, 0.],
             [0., 0.01234568, 0.60493827, 1., 0.],
             [0.60493827, 0., 0., 0., 1.]])
        self.adapt_bisquare_kern = np.array(
            [[
                1.00000000e+00, 0.00000000e+00, 0.00000000e+00, 3.99999881e-14,
                7.15976383e-01
            ],
             [
    ...
    def test_fix_gauss(self):
        kern = np.array([
            Kernel(i, self.coords, 3, function="gaussian", fixed=True).kernel
    ...
    def test_adapt_gauss(self):
        kern = np.array([
            Kernel(i, self.coords, 3, function="gaussian", fixed=False).kernel
            for i in range(self.n)
        ])
    ...
    def test_fix_biqsquare(self):
        kern = np.array([
            Kernel(i, self.coords, 3, function="bisquare", fixed=True).kernel
            for i in range(self.n)
        ])
        np.testing.assert_allclose(kern, self.fix_bisquare_kern, atol=1e-01)

    def test_adapt_bisqaure(self):
        kern = np.array([
            Kernel(i, self.coords, 3, function="bisquare", fixed=False).kernel
            for i in range(self.n)
        ])
    ...
    def test_fix_exp(self):
        kern = np.array([
            Kernel(i, self.coords, 3, function="exponential",
                   fixed=True).kernel for i in range(self.n)
        ])
    ...
    def test_adapt_exp(self):
        kern = np.array([
            Kernel(i, self.coords, 3, function="exponential",
                   fixed=False).kernel for i in range(self.n)
        ])
